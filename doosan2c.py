            ...
            if data:
                ...
                if classification == size:
                    ...
                    if status == 1:
                        ...
                        time.sleep(6)
                        sock.sendall(b"movej(posj(121.9, 15.8, 75.8, 181.1, -84.3, -9.2), v=20, a=20)")
                        time.sleep(7)
                        sock.sendall(
                            f"movel(posx({x}, {y}, {z+zz}, 168.5, -177.8, -11.9), v=80, a=80)".encode())
                        time.sleep(8)
                        sock.sendall(
                            f"movel(posx({x}, {y}, {z}, 168.5, -177.8, -11.9), v=80, a=80)".encode())
                        time.sleep(10)
                        ...
                        sock.sendall(
                            f"movel(posx({x}, {y}, {z + zz}, 168.5, -177.8, -11.9), v=80, a=80)".encode())
                        time.sleep(13)
                        sock.sendall(b"movel(posx(-135.9, 609.5, 509.1,  168.8, -178.0, -11.6), v=80, a=80)")
                        time.sleep(15)
                        status = 0
                        sock.sendall(b"movel(posx(559.0, -56.8, 661.1, 169.3, 180.0, -10.7), v=300, a=300)")
                        time.sleep(16)
                        print(f'boxcount:{boxcount}')
                        boxcount = boxcount + 1
                        print(f'boxcount:{boxcount}')
        ...
        for i in range(1):
            ...
            if data:
                ...
                # =========== MVT FOR MEDIUM =============#
                    ...
                    time.sleep(3)
                    sock.sendall(
                        f"movel(posx(545.8, {xx}, 245.8, 151.3, -176.5, -28.9), v=40, a=40)".encode())  ### get medium
                    time.sleep(5)
                    status = 2
                    print(f"stattt: {status}")
                    if status == 2:
